...
async function listedArtifact(deliveryId: string): Promise<ArtifactView> {
  const priorWakes = [...wakes]
  const result = await request<{ artifacts: ArtifactView[] }>('/external-artifacts')
  assert.equal(result.status, 200, JSON.stringify(result.value))
  assert.deepEqual(wakes, priorWakes)
  // Discovery returns durable workflow metadata, never evidence or report snapshots.
  assert.doesNotMatch(JSON.stringify(result.value), /"(?:body|citations|limitations)":/)
  const artifact = result.value.artifacts.find(item => item.versions.some(version => version.sourceDeliveryId === deliveryId))
  assert.ok(artifact, 'saved source delivery must be discoverable without an artifact ID')
  return artifact
}
...
test('fixed full evidence -> one explicit native task -> immutable draft -> human approval', async () => {
  ...
  const savedList = await listedArtifact(deliveryId)
  assert.equal(savedList.id, artifact.id)
  assert.equal(savedList.versions.find(version => version.sourceDeliveryId === deliveryId)?.version, 1)
  assert.deepEqual(savedList.handoffs, [])
  ...
  const assignedList = await listedArtifact(deliveryId)
  assert.equal(assignedList.handoffs[0].id, assigned.id)
  assert.equal(assignedList.handoffs[0].status, 'assigned')
  assert.equal(assignedList.handoffs[0].sourceVersion, 1)
  assert.equal(assignedList.handoffs[0].assigneeId, reporterId)
  await assert.rejects(service.list({ companyId, subjectId: reporterId }), /artifact_owner_required/)
  ...
  const submittedList = await listedArtifact(deliveryId)
  assert.equal(submittedList.handoffs[0].status, 'submitted')
  assert.equal(submittedList.handoffs[0].outputVersion, output.version)
  const recoveredDraft = await request<ArtifactContent>(`/external-artifacts/${submittedList.id}/versions/${submittedList.handoffs[0].outputVersion}`)
  assert.equal(recoveredDraft.status, 200)
  assert.equal(recoveredDraft.value.body, draft)
  ...
  const reviewedList = await listedArtifact(deliveryId)
  assert.equal(reviewedList.handoffs[0].id, assigned.id)
  assert.equal(reviewedList.handoffs[0].status, 'accepted')
  assert.equal(reviewedList.handoffs[0].outputVersion, output.version)
  assert.equal(reviewedList.handoffs[0].reviewNote, 'Synthetic human fixture confirms this fixed draft.')
...
test('input revisions stale pending work, reject late output, and require a matching new external answer', async () => {
  ...
  assert.equal((await readNative(artifact)).value.ok, false)
  const recovered = await listedArtifact(artifact.versions[0].sourceDeliveryId!)
  assert.equal(recovered.id, artifact.id)
  assert.equal(recovered.versions.find(version => version.sourceDeliveryId === artifact.versions[0].sourceDeliveryId)?.stale, true)
  assert.equal(recovered.versions.find(version => version.sourceDeliveryId === next.value.versions.at(-1)!.sourceDeliveryId)?.version, next.value.latestVersion)
  assert.equal(recovered.handoffs.find(item => item.id === assigned.id)?.status, 'stale')
  assert.equal(recovered.handoffs.find(item => item.id === newHandoff.id)?.status, 'assigned')
})
...
test('cancellation revokes access and submission; neither retries nor unrelated actors can widen the grant', async () => {
  ...
  assert.deepEqual(await service.list({ companyId, subjectId: 'u-other-human' }), [])
  await pool.query("INSERT INTO companies(id,name,slug,owner_user_id) VALUES('c-artifact-foreign','Foreign','artifact-foreign',$1)", [userId])
  await seedUserMembership(userId, 'c-artifact-foreign')
  const foreignList = await request<{ artifacts: ArtifactView[] }>('/external-artifacts', undefined, { 'x-company-id': 'c-artifact-foreign' })
  assert.equal(foreignList.status, 200)
  assert.deepEqual(foreignList.value.artifacts, [])
...
test('assignment changes and source membership loss invalidate already-issued read grants', async () => {
  const artifact = await capture()
  await handoff(artifact)
  assert.equal((await listedArtifact(artifact.versions[0].sourceDeliveryId!)).handoffs[0].status, 'assigned')
  ...
  const revokedList = await request<{ artifacts: ArtifactView[] }>('/external-artifacts')
  assert.equal(revokedList.status, 200)
  assert.deepEqual(revokedList.value.artifacts, [])
  assert.deepEqual(wakes, [reporterId])
  assert.equal(fixture.requests.filter(r => r.path.includes('/agent-chat/')).length, 1)
})
...
test('expiration removes content, not identity fences, and never resubmits the external request', async () => {
  ...
  const redacted = await listedArtifact(artifact.versions[0].sourceDeliveryId!)
  assert.equal(redacted.expired, true)
  assert.equal(redacted.inputText, '')
  assert.equal(redacted.handoffs[0].instructions, '')
  assert.equal(redacted.handoffs[0].reviewNote, null)
  const { runDbGcTick } = await import('../db-gc.js')
  await runDbGcTick()
  const expired = await listedArtifact(artifact.versions[0].sourceDeliveryId!)
